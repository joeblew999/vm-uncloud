...
|---|---|
| [PR #385](https://github.com/psviderski/uncloud/pull/385) — compose `secrets` (`/run/secrets`) | (a) inject the CF DNS-01 token + `MOLTIS_TOKEN` as secret **files** instead of `${VAR}` env → likely sidesteps the [#393](https://github.com/psviderski/uncloud/issues/393) trailing-`\n`, so we drop the `tr -d '\r\n'` trim in `caddy/compose.yaml` (caveat: confirm the secret *file* itself has no trailing `\n`). (b) resolve secrets **client-side** (fnox→OrangeVault) and inject only resolved values → drop the in-container `ov-bootstrap`/`bw` + the **dev/CI account unlock creds on the node**. See [compose-secrets.md](compose-secrets.md). Commented. |
...
| [PR #390](https://github.com/psviderski/uncloud/pull/390) — `uc machine exec` / `uc machine ssh` | the raw `ssh root@<ip>` in `vultr-snapshot.nu` (stream qcow2→R2), debug probes, the viewer tunnel — uncloud already holds the connection, so no separate key/host handling. Commented use cases + security. |
